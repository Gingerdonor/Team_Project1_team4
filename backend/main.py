...
ADMIN_USERNAMES = {
    "admin",
    "administrator",
}  # 관리자 사용자명 목록 (테스트용, 프로덕션에서는 DB로 관리 권장)
...
@app.get("/api/admin/analysis-results/{result_id}")
...
@app.put("/api/admin/analysis-results/{result_id}")
...
# --- MBTI Celebrities 관리 API ---


@app.get("/api/admin/celebrities")
...
@app.get("/api/admin/celebrities/{celebrity_id}")
...
@app.post("/api/admin/celebrities")
def admin_create_celebrity(
    ...
    return {
        "message": "유명인이 추가되었습니다.",
        "id": new_celebrity.id,
    }


@app.put("/api/admin/celebrities/{celebrity_id}")
...
@app.delete("/api/admin/celebrities/{celebrity_id}")
